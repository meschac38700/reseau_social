...
<div class="container" style="padding-top: 2em ">
	...
	<form action="" method="POST" >
		...
		<div class="form-group">
			...
			<input  value="<?= $last_name ?? '' ?>" type="text" class="form-control <?php echo has_error( "last_name", $error_fields) ?>" placeholder="Entre votre prenom..." required="required" name="last_name" id="last_name">
			<?php error_span("last_name", $error_fields) ?>
		</div>
		<!-- Le champ prenom -->
		<div class="form-group">
			<label for="first_name" class="label-control"><?=$lang['form']['field']['first_name']?></label>
			<input value="<?= $first_name?? '' ?>" type="text" class="form-control <?php echo has_error( "first_name", $error_fields) ?>" placeholder="Entre votre nom..." required="required" name="first_name" id="first_name">
			<?php error_span("first_name", $error_fields) ?>
		</div>
		<!-- Le champ pseudo -->
		<div class="form-group">
			<label for="pseudo" class="label-control"><?=$lang['form']['field']['pseudo']?></label>
			<input  value="<?= $pseudo ?? '' ?>" type="text" class="form-control <?php echo has_error( "pseudo", $error_fields) ?>" placeholder="Entre votre pseudo..." required="required" name="pseudo" id="pseudo">
			<?php error_span("pseudo", $error_fields) ?>
		</div>
		<!-- Le champ email -->
		<div class="form-group">
			<label for="email" class="label-control"><?=$lang['form']['field']['email']?></label>
			<input  value="<?= $email ?? '' ?>" type="email" class="form-control <?php echo has_error( "email", $error_fields) ?>" placeholder="Entre votre adresse email..." required="required" name="email" id="email">
		...
		<div class="form-group">
			...
			<input  value="<?= $password_confirm ?? '' ?>" type="password" class="form-control <?php echo has_error( "password_confirm", $error_fields) ?>" placeholder="Entre votre mot de passe..." required="required" name="password_confirm" id="password_confirm">
